...
{
	if(Head != NULL && Head->aCurrentSize > 1)
	{
		for(int i = 0; i < Head->aCurrentSize - 1; i++)
	    {
	    	for(int j = 0; j < Head->aCurrentSize - 1 - i; j++)
	        {
	        	if(aSquare(Head->theList[j]) > aSquare(Head->theList[j + 1]))
	            {
	            	Piramide *tmp = Head->theList[j];
	            	Head->theList[j] = Head->theList[j + 1];
	            	Head->theList[j + 1] = tmp;
	            }
	        }
	    }
	}
