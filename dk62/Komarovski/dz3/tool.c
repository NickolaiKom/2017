...
void printJson(List *Head)
{
	if(Head != NULL)
	{
		FILE *afile = fopen("txt.json","w");
		
		if(afile != NULL)
		{
			fprintf(afile, "{\"Max size\":%i,", Head->aSize);
			fprintf(afile, "\"Current size\":%i", Head->aCurrentSize);
			fprintf(afile, ",\"Piramides\":[\n");
			
			for(int i = 0; i < Head->aCurrentSize; i++)
			{
				fprintf(afile, "{\n");
				printPirJson(Head, afile, i);
				fprintf(afile, "}\n");
				
				if(i + 1 != Head->aCurrentSize)
					fprintf(afile, ",\n");
			}
			fprintf(afile, "]}\n");
...
{
	if(Head != NULL && afile != NULL)
	{
		Piramide *object = Head->theList[i];
		
		fprintf(afile, "\t\"Dot A\": {\"X\":%i,\t\"Y\":%i,\t\"Z\":%i},\n", object->A->x, object->A->y, object->A->z);
		fprintf(afile, "\t\"Dot B\": {\"X\":%i,\t\"Y\":%i,\t\"Z\":%i},\n", object->B->x, object->B->y, object->B->z);
		fprintf(afile, "\t\"Dot C\": {\"X\":%i,\t\"Y\":%i,\t\"Z\":%i},\n", object->C->x, object->C->y, object->C->z);
		fprintf(afile, "\t\"Dot D\": {\"X\":%i,\t\"Y\":%i,\t\"Z\":%i}\n", object->D->x, object->D->y, object->D->z);
	}
}
