...
void shellSort(List *Head)
{
	if(Head != NULL)
	{
		...
		while(H <= Head->aCurrentSize / 3)
			H = H * 3 + 1;
		
		while(H > 0)
		{
			for(j = H; j < Head->aCurrentSize; j++)
			{
				Piramide *aTmp = Head->theList[j];
				i = j;
				
				while(i > H - 1 && aSquare(Head->theList[i - H]) >= aSquare(aTmp))
				{
					(Head->theList[i]) = (Head->theList[i - H]);
					i -= H;
				}
				(Head->theList[i]) = (aTmp);
				aTmp = NULL;
...
int binarySearch(List *Head)
{
	if(Head != NULL)
	{
		int middle, left = 0;
		int right = Head->aCurrentSize - 1;
		double toFind;
		
		printf("\nEnter square to search: ");
		scanf("%lf", &toFind);
	    ...
	    while(left <= right)
	    {
	        ...
	        if(aSquare(Head->theList[middle]) == toFind)
	            return middle;
	        
	        else if(aSquare(Head->theList[middle]) > toFind)
	        	right = middle - 1;
